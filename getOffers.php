...
function getHTMLOffer($dbConn) {
	try {
	    ...
	    $sql = "select concat('<p>&#8220;',bookTitle,'&#8221;<br />\n<span class=\"category\">Category: ',catDesc,'</span><br />\n<span class=\"price\">Price: ',bookPrice,'</span></p>') as offer from nbc_special_offers inner join nbc_category on nbc_special_offers.catID = nbc_category.catID order by rand() limit 1";
...
function getJSONOffer($dbConn) {
	try {
	    $sql = "select bookTitle, catDesc, bookPrice from nbc_special_offers inner join nbc_category on nbc_special_offers.catID = nbc_category.catID order by rand() limit 1";
	   	$rsOffer = $dbConn->query($sql);;
	    $offer = $rsOffer->fetchObject();
